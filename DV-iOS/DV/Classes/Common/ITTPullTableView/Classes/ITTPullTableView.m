...
- (void)setPullTableIsRefreshing:(BOOL)isRefreshing
{
    if(!pullTableIsRefreshing && isRefreshing) {
        ...
        [refreshView performSelector:@selector(egoRefreshScrollViewDataSourceDidFinishedLoading:) withObject:self afterDelay:0.3f];
        pullTableIsRefreshing = NO;
    }
}
...
{
    [refreshView startRefresh:self];
}

- (void)setRefreshToLoadMore;
{
    refreshView.refreshToLoadMore = YES;
}
