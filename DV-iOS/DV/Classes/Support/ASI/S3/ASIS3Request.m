...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setPersistentConnectionTimeoutSeconds:60];
...
- (NSMutableDictionary *)S3Headers
{
	...
	if ([self accessPolicy]) {
		headers[@"x-amz-acl"] = [self accessPolicy];
	}
	return headers;
}
...
- (void)buildRequestHeaders
{
	...
	for (NSString *header in [amzHeaders keysSortedByValueUsingSelector:@selector(compare:)]) {
		canonicalizedAmzHeaders = [NSString stringWithFormat:@"%@%@:%@\n",canonicalizedAmzHeaders,[header lowercaseString],amzHeaders[header]];
		[self addRequestHeader:header value:amzHeaders[header]];
	}
...
{
	if ([[self responseHeaders][@"Content-Type"] isEqualToString:@"application/xml"]) {
...
{
	[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIS3ResponseParsingFailedType userInfo:@{NSLocalizedDescriptionKey: @"Parsing the resposnse failed",NSUnderlyingErrorKey: parseError}]];
}
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
	...
	if ([elementName isEqualToString:@"Message"]) {
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIS3ResponseErrorType userInfo:@{NSLocalizedDescriptionKey: [self currentXMLElementContent]}]];
...
+ (NSDateFormatter*)S3ResponseDateFormatter
{
	...
	NSDateFormatter *dateFormatter = threadDict[@"ASIS3ResponseDateFormatter"];
	if (dateFormatter == nil) {
		dateFormatter = [[[NSDateFormatter alloc] init] autorelease];
		[dateFormatter setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"] autorelease]];
		[dateFormatter setTimeZone:[NSTimeZone timeZoneWithAbbreviation:@"UTC"]];
		[dateFormatter setDateFormat:@"yyyy-MM-dd'T'HH:mm:ss'.000Z'"];
		threadDict[@"ASIS3ResponseDateFormatter"] = dateFormatter;
	}
...
{
	NSMutableDictionary *threadDict = [[NSThread currentThread] threadDictionary];
	NSDateFormatter *dateFormatter = threadDict[@"ASIS3RequestHeaderDateFormatter"];
	if (dateFormatter == nil) {
		dateFormatter = [[[NSDateFormatter alloc] init] autorelease];
		...
		threadDict[@"ASIS3RequestHeaderDateFormatter"] = dateFormatter;
	}
	return dateFormatter;
