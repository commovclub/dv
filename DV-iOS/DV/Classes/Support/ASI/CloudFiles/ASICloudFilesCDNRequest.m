...
- (BOOL)cdnEnabled {
    NSNumber *enabled = [self responseHeaders][@"X-CDN-Enabled"];
    if (!enabled) {
        enabled = [self responseHeaders][@"X-Cdn-Enabled"];
    }
	return [enabled boolValue];
}

- (NSString *)cdnURI {
	NSString *uri = [self responseHeaders][@"X-CDN-URI"];
    if (!uri) {
        uri = [self responseHeaders][@"X-Cdn-Uri"];
    }
    return uri;
}

- (NSString *)cdnSSLURI {
    NSString *uri = [self responseHeaders][@"X-CDN-SSL-URI"];
    if (!uri) {
        uri = [self responseHeaders][@"X-Cdn-Ssl-Uri"];
    }
	return uri;
}

- (NSUInteger)cdnTTL {
    NSNumber *ttl = [self responseHeaders][@"X-TTL"];
    if (!ttl) {
        ttl = [self responseHeaders][@"X-Ttl"];
    }
    return [ttl intValue];
}
