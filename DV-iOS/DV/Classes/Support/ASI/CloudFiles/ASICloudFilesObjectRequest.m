...
- (NSUInteger)containerObjectCount {
	return [[self responseHeaders][@"X-Container-Object-Count"] intValue];
}

- (NSUInteger)containerBytesUsed {
	return [[self responseHeaders][@"X-Container-Bytes-Used"] intValue];
}
...
- (ASICloudFilesObject *)object {
	...
	object.name = path;
	object.hash = [self responseHeaders][@"ETag"];
	object.bytes = [[self responseHeaders][@"Content-Length"] intValue];
	object.contentType = [self responseHeaders][@"Content-Type"];
	object.lastModified = [self responseHeaders][@"Last-Modified"];
	...
	for (NSString *key in [[self responseHeaders] keyEnumerator]) {
		...
		if (metaRange.location == 0) {
			(object.metadata)[[key substringFromIndex:metaRange.length]] = [self responseHeaders][key];
		}
	}
...
+ (id)putObjectRequestWithContainer:(NSString *)containerName objectPath:(NSString *)objectPath contentType:(NSString *)contentType objectData:(NSData *)objectData metadata:(NSDictionary *)metadata etag:(NSString *)etag {
	...
	if (metadata) {
		for (NSString *key in [metadata keyEnumerator]) {
			[request addRequestHeader:[NSString stringWithFormat:@"X-Object-Meta-%@", key] value:metadata[key]];
		}
	}	
