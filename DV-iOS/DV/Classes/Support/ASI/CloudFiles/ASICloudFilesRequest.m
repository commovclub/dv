...
+ (NSError *)authenticate
{
	...
	if (![request error]) {
		...
		storageURL = responseHeaders[@"X-Storage-Url"];
		cdnManagementURL = responseHeaders[@"X-CDN-Management-Url"];
...
-(NSDate *)dateFromString:(NSString *)dateString
{
	...
	NSDateFormatter *dateFormatter = threadDict[@"ASICloudFilesResponseDateFormatter"];
	if (dateFormatter == nil) {
		dateFormatter = [[[NSDateFormatter alloc] init] autorelease];
		[dateFormatter setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"] autorelease]];
		// example: 2009-11-04T19:46:20.192723
		[dateFormatter setDateFormat:@"yyyy-MM-dd'T'H:mm:ss.SSSSSS"];
		threadDict[@"ASICloudFilesResponseDateFormatter"] = dateFormatter;
	}
