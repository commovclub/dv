...
- (void)addContact:(id)contact withName:(NSString *)name {
    ...
    if ([self.contactKeys containsObject:contact]){
        ...
        [[HTActivityIndicator currentIndicator] displayMessage:@"该标签已存在，请不要重复添加"];
        return;
    }
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text;
{
    ...
    if ( [text isEqualToString:@"\n"] ) { // Return key was pressed
        if ([[textView.text stringByTrimmingCharactersInSet:
              [NSCharacterSet whitespaceCharacterSet]] length]>0) {
            [self addContact:textView.text withName:textView.text];
