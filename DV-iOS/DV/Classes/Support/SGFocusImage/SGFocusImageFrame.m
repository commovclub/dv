...
- (id)initWithFrame:(CGRect)frame delegate:(id<SGFocusImageFrameDelegate>)delegate images:(NSMutableArray*)images
{
    self = [super initWithFrame:frame];
    if (self) {
        NSMutableArray *imageItems = [NSMutableArray array];
        ...
        for (int i =0; i<[images count]; i++) {
            [imageItems addObject: [images objectAtIndex:i]];
        }
...
- (void)setupViews
{
    ...
    for (int i = 0; i < imageItems.count; i++) {
        ...
        //添加背景图片
        //ITTImageView *background =[[ITTImageView alloc] init];
        //[background loadImage:item.url placeHolder:[UIImage imageNamed:@"p3.png"]];
        [item.background setFrame:CGRectMake(i * _scrollView.frame.size.width, 0, _scrollView.frame.size.width, _scrollView.frame.size.height)];
        [item.background setContentMode: UIViewContentModeScaleAspectFill];
        ...
        [_scrollView addSubview:item.background];
