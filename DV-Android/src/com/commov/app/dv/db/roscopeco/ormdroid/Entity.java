...
/**
 * <p>
 * Base class for persistent entities. The only hard requirements for model
 * classes are that they subclass this class, and that they provide a (currently
 * integral) primary key (see below).
 * </p>
 * 
 * <p>
 * <code>Entity</code> is the primary class in ORMDroid, and is where most
 * interaction with the API will take place. A model class will subclass this
 * class, and will inherit its save() and delete() methods from it. The simplest
 * possible model class would be:
 * </p>
 * 
 * <p>
 * 
 ...
 * 
 * <p>
 * This is obviously useless, as it holds no data other than the (required)
 * primary key. In order to actually store other data, you would add further
 * public fields. Entities may also define any methods you wish - these are
 ...
 * 
 * <p>
 * By default, the framework creates table names based on the fully-qualified
 * name of the entity class. You can change this behaviour by applying the
 * {@link Table} annotation to the class, e.g:
 * </p>
 * 
 * <p>
 * 
 ...
 * 
 * <p>
 * Similarly, any column can be explicitly named using the {@link Column}
 * annotation:
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * In the example above, the framework automatically selects the primary key
 * field based on it's name. Currently, the framework will use the first field
 * it finds named 'id' or '_id', and will map these fields to database column
 * '_id' in any case, unless the field is explicitly named (see above).
 * </p>
 * 
 * <p>
 * It is possible to explicitly select the primary key column using the
 ...
 * 
 * <p>
 * It should be noted that, although you can use any data type for primary key
 * fields, parts of the framework currently expect primary keys to be integers,
 * and will behave in an indeterminite manner if other types are used. This
 * limitation primarily affects the {@link Query} class, and the
 * {@link #equals(Object)} and {@link #hashCode()} implementations defined in
 * this class, and will be removed in a future version.
 * </p>
 * 
 * <h3>Relationships</h3>
 * 
 * <p>
 * The framework currently provides built-in support for one-to-one
 * relationships - simply adding a field of an <code>Entity</code>-subclass type
 * will cause that field to be persisted when the containing object is
 * persisted.
 * </p>
 * 
 * <p>
 * Many relationships are not currently natively supported, but can easily be
 * implemented using helper methods on your model class. For example (taken from
 * the ORMSample app):
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * More support for such relationships (including entity type mappings for
 * {@link java.util.List} and {@link java.util.Map}) will be added in a future
 * version.
 * </p>
 * 
 * <p>
 * If you have a bidirectional relationship, you must annotate one side of that
 * relationship with the <code>{@literal @}{@link Column}(inverse = true)</code>
 * annotation to prevent infinite loops when persisting your data. This will
 * prevent the inverse field from being persisted when it's model is stored.
 * </p>
 * 
 * <h3>Model lifecycle</h3>
 * 
 * <p>
 * The typical lifecycle of a model is shown below:
 * </p>
 * 
 * <p>
 * 
 ...
 * 
 * <p>
 * The default implementation of equals and hashCode provided by this class
 * define equality in terms of primary key, and utilise reflective field access.
 * You may of course override these if you wish to change their behaviour, or
 * for performance reasons (e.g. to directly compare primary key fields rather
...
public abstract class Entity {
	static final class EntityMapping {
		...
		private static final Pattern MATCH_DOTDOLLAR = Pattern
				.compile("[\\.\\$]");
		...
		static EntityMapping build(Class<? extends Entity> clz) {
			...
			if (table != null) {
				...
				mapping.mTableName = MATCH_DOTDOLLAR.matcher(clz.getName())
						.replaceAll("");
			}

			ArrayList<String> seenFields = new ArrayList<String>();
			for (Field f : clz.getFields()) {
				// Blithely ignore this field if we've already seen one with
				// same name -
				// Java field hiding allows this to happen and if it does,
				// without this
				// we'd be adding the same column name twice.
				//
				// We might as well also ignore it here if it's inverse, since
				// we'll
				...
				if (!Modifier.isStatic(modifiers)
						...
						&& !seenFields.contains(f.getName()) && !inverse) {
			...
			if (mapping.mPrimaryKey == null) {
				...
				throw new ORMDroidException(
						"No primary key was specified, and a default could not be determined for "
								+ clz);
			}
		...
		Object getPrimaryKeyValue(Entity o) {
			try {
				...
				Log.e(TAG, "IllegalAccessException accessing primary key "
						+ mPrimaryKey + "; Update failed");
				throw new ORMDroidException(
						"IllegalAccessException accessing primary key "
								+ mPrimaryKey + "; Update failed");
			}
		}
			...
			for (int i = 0; i < len; i++) {
				...
				if (name != mPrimaryKeyColumnName) {
					...
					try {
						...
						Log.w(TAG, "IllegalAccessException accessing field "
								+ fields.get(i).getName() + "; Inserting NULL");
						val = null;
					}
					b.append(val == null ? "null" : processValue(db, val));
		...
		int insert(SQLiteDatabase db, Entity o) {
			...
			try {
				...
				try {
					if (o.getClass().getSimpleName().equals("SJA")) {
						db.execSQL("alter table commellorahseqmodelsSJA add conclusion TEXT");
					} else if (o.getClass().getSimpleName().equals("IA")) {
						db.execSQL("alter table commellorahseqmodelsIA add conclusion TEXT");
					}
					db.execSQL(sql);
		...
		void update(SQLiteDatabase db, Entity o) {
			String sql = "UPDATE " + mTableName + " SET " + getSetFields(db, o)
					+ " WHERE " + mPrimaryKeyColumnName + "="
					+ getPrimaryKeyValue(o);
		...
		<T extends Entity> T load(SQLiteDatabase db, Cursor c) {
			try {
				...
				for (int i = 0; i < len; i++) {
					...
					if (colIndex == -1) {
						Log.e("Internal<ModelMapping>",
								"Got -1 column index for `"
										+ colNames.get(i)
										+ "' - Database schema may not match entity");
						throw new ORMDroidException("Got -1 column index for `"
								+ colNames.get(i)
								+ "' - Database schema may not match entity");
					} else {
						Object o = TypeMapper.getMapping(f.getType())
								.decodeValue(db, ftype, c, colIndex);
						f.set(model, o);
					}
				}
	...
	/**
	 * <p>
	 * Create a new {@link Query} that will query against the table mapped to
	 * the specified class.
	 * </p>
	 * 
	 * <p>
	 * See the {@link Query} documentation for examples of usage.
	 * </p>
	 * 
	 * @param clz
	 *            The class to query.
	...
	/**
	 * <p>
	 * Determine whether this instance is backed by the database.
	 * </p>
	 * 
	 * <p>
	 * <strong>Note</strong> that a <code>false</code> result from this method
	 * does <strong>not</strong> indicate that the data in the database is up to
	 * date with respect to the object's fields.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Get the value of the primary key field for this object.
	 * </p>
	 * 
	 * <p>
	 * Note that this currently uses reflection.
	 * </p>
	 * 
	...
	public boolean equals(Object other) {
		...
		return other != null
				...
				&& ((Entity) other).getPrimaryKeyValue().equals(
						getPrimaryKeyValue());
	}
	...
	@Override
	public int hashCode() {
		// TODO this uses reflection. Also, could act wierd if non-int primary
		// keys...
