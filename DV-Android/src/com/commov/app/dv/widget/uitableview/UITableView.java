...
public class UITableView extends RelativeLayout {
	...
	if (mItemList.size() > 1) {
		...
		if (mIndexController == 0) {
		    ...
		    itemView = mInflater.inflate(R.layout.list_item_bottom,
			    null);
		} else {
		    itemView = mInflater.inflate(R.layout.list_item_middle,
			    null);
		}
	...
	if (item.getTitle() != null) {
	    ...
	    titleTextView.setText(item.getTitle().toString());
	    titleTextView.setText(item.getTitle());
	    if (item.isHightlight()) {
		//titleTextView.setTextColor(R.color.dv_blue);
