...
import com.commov.app.dv.utils.ImageUtil;
...
public class ImageViewRounded extends ImageView {
	  ...
	  protected void onDraw(Canvas canvas) {
	    ...
	    // If the rounded corner bitmap is already set to the drawable, then we call
	    ...
	    if (drawable != null) {
	      ...
	      if (viewWidth != 0f) { // Only round the corner if view width is not zero.
	        ...
	        if (bitmapCornerRadius != newBitmapCornerRadius) {
	          ...
	          bitmap = ImageUtil.getRoundedCornerBitmap(bitmap, bitmapCornerRadius,
	              bitmapWidth, bitmapHeight, false, false, false, false);
