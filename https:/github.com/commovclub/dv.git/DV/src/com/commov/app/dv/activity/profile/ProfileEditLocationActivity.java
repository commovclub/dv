...
public class ProfileEditLocationActivity extends BaseActivity implements
        OnClickListener {
    ...
    private int[] countyOfGuoWai = {R.array.guowai_city_item};
    ...
    private void loadSpinner() {
        ...
        province_spinner.setPrompt("选择您的省");
    ...
    private void sendProfile() {
        showLoadingDialog();
        String value = province_spinner.getSelectedItem().toString() + " "
        ...
        String url = ServerAPI.prefix + "member/"
                + sph.getValue(SharedPreferencesHelper.USERID) + "/updateCity";
        ConnectionHelper conn = ConnectionHelper.obtainInstance();
        List<NameValuePair> nvp = new ArrayList<NameValuePair>();
        nvp.add(new BasicNameValuePair("city", value));
        ...
        public void onResult(int resultCode, int requestId, String rawResponses) {
            ...
            if (requestId == 0 && resultCode == RESULT_STATE_OK) {
                if (rawResponses != null && rawResponses.length() > 0) {
                    Utils.Toast(ProfileEditLocationActivity.this, "提交成功！");
                    Intent intent = new Intent();
                    intent.putExtra("contact", contact);
                    ProfileEditLocationActivity.this.setResult(RESULT_OK,
                            intent);
                    finish();
                    return;
                }
            }
            Utils.simpleNetBadNotify(ProfileEditLocationActivity.this);
        }
    };
}