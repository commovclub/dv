 ...
 * 
 * In fact, to implement Segmented control, you don't need to do any thing, but
 * just customize the background attribute of the RadioButtons (no need to use
 ...
 * 
 * To get the Segmented control, use <code>SegmentedControlView</code> instead
 * of the RadioGroup. Each RadioButton is a button in a button of the Segmented
 ...
 * middle buttons.</li>
 * <li><code>lastButtonBackground</code>: The background attribute for the last
 * button.</li>
 ...
 * 
 * This implementation is referenced to
...
public class SegmentedControlView extends RadioGroup {
	...
	public SegmentedControlView(Context context, AttributeSet attrs) {
		...
		backgroundFirstButton = a
				.getDrawable(R.styleable.SegmentedControlView_firstButtonBackground);
		backgroundLastButton = a
				.getDrawable(R.styleable.SegmentedControlView_lastButtonBackground);
		backgroundMiddleButton = a
				.getDrawable(R.styleable.SegmentedControlView_middleButtonBackground);
		backgroundUniqueButton = a
				.getDrawable(R.styleable.SegmentedControlView_uniqueButtonBackground);

		// Set the orientation to horizontal by default.
		int orientation = a.getInt(
				R.styleable.SegmentedControlView_android_orientation,
				HORIZONTAL);
	...
	private void changeButtonsImages() {
		...
		if (count > 1) {
			...
			if (backgroundLastButton != null) {
				getChildAt(count - 1).setBackgroundDrawable(
						backgroundLastButton);
			}
		} else if (count == 1) {
